...
exports.getARSummary = async (qSearch = "", tenantId) => {
  const searchTerm = (qSearch || "").toString().trim().toLowerCase();
  ...
  const q = `
    WITH order_totals AS (
      ...
      WHERE o.status <> 'ANULADO' AND COALESCE(o.terms, 'CONTADO') = 'CREDITO'
    ...
    SELECT
      ...
      COALESCE(SUM(ot.order_total), 0) AS total_sold,
  ...
  const r = await db.query(q, [searchTerm], tenantId);
  return r.rows;
};