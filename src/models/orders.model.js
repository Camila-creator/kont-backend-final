// src/models/orders.model.js — VERSION FINAL CORREGIDA
const db = require("../db");
...
async function getTenantConfig(tenantId) {
  try {
    ...
    console.warn("Error obteniendo config del tenant:", err.message);
    return { has_imei: false, has_production: false };
  }
}

// ─────────────────────────────────────────────────────────
// NUEVA: Validar stock antes de confirmar (evita stock negativo)
// ─────────────────────────────────────────────────────────
async function validateStockForOrder(orderId, tenantId, config) {
  ...
  for (const it of itemsR.rows) {
    if (config.has_production) {
      if (Number(it.stock) < Number(it.qty)) {
        errors.push(`"${it.name}": disponible ${it.stock}, necesitas ${it.qty}`);
      }
    } else {
      const recipeR = await db.query(
        `SELECT id FROM recipes WHERE product_id = $1 AND tenant_id = $2`,
        [it.product_id, tenantId], tenantId
      );
      if (recipeR.rows.length > 0) {
        const compsR = await db.query(
          `SELECT ri.qty AS comp_qty, s.stock, s.name AS sname
           FROM recipe_items ri JOIN supplies s ON s.id = ri.supply_id
           WHERE ri.recipe_id = $1 AND ri.tenant_id = $2`,
          [recipeR.rows[0].id, tenantId], tenantId
        );
        for (const c of compsR.rows) {
          const needed = Number(c.comp_qty) * Number(it.qty);
          if (Number(c.stock) < needed) {
            errors.push(`Insumo "${c.sname}": disponible ${c.stock}, necesitas ${needed}`);
          }
        }
      } else {
        if (Number(it.stock) < Number(it.qty)) {
...
// ─────────────────────────────────────────────────────────
// BUG 2 FIX: leer recipe_items (no recipes directamente)
// ─────────────────────────────────────────────────────────
  ...
  for (const it of itemsR.rows) {
    ...
    if (config.has_production) {
      await db.query(
        ...
        [qty, it.product_id, tenantId], tenantId
      );
    } else {
      try {
        const recipeR = await db.query(
        ...
        if (recipeR.rows.length > 0) {
          // BUG 2 FIX: leer de recipe_items con supply_id correcto
          const compsR = await db.query(
          ...
          for (const comp of compsR.rows) {
            await db.query(
              ...
              [Number(comp.comp_qty) * qty, comp.supply_id, tenantId], tenantId
            );
          }
        } else {
          await db.query(
        ...
        console.warn(`Error descuento inventario producto ${it.product_id}:`, err.message);
      }
    }
  }
}
...
async function getOrderFullById(id, tenantId) {
  try {
    const orderR = await db.query(
      `SELECT o.*, c.name AS customer_name, c.doc AS customer_doc,
    ...
    const paymentsR = await db.query(
    ...
    console.error("Error en getOrderFullById:", err.message);
    throw err;
  }
}

async function createOrder(data, tenantId, user) {
  const { customer_id, status, terms, price_mode, wholesale_threshold, notes, order_date, items, payments, discount_amount, exchange_rate } = data;

  if ((items || []).length > 500) throw new Error("Un pedido no puede tener más de 500 items.");

  const config = await getTenantConfig(tenantId);
  const finalStatus = (status || "BORRADOR").toUpperCase();

  const orderR = await db.query(
    `INSERT INTO orders (tenant_id, customer_id, status, terms, price_mode, wholesale_threshold, notes, order_date, discount_amount, exchange_rate, order_number)
  ...
  for (const it of items || []) {
    ...
    await db.query(
      `INSERT INTO order_items (order_id, product_id, qty, unit_price, total, tenant_id)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [order.id, it.product_id, qty, unit, qty * unit, tenantId], tenantId
    );
    if (config.has_imei && it.serials?.length > 0) {
      for (const imei of it.serials) {
        const clean = imei.toUpperCase().trim();
        try {
          await db.query(
            `UPDATE serial_numbers SET status = 'VENDIDO', order_id = $3 WHERE imei = $1 AND tenant_id = $2`,
            [clean, tenantId, order.id], tenantId
          );
        } catch (e) { console.warn(`Error serial ${clean}:`, e.message); }
      }
    }
  }

  // LÓGICA 5 FIX: EQUIPO_USADO NO va a customer_payments
  for (const p of payments || []) {
    if (p.method === "EQUIPO_USADO") {
      ...
      await db.query(
        ...
        [tenantId, customer_id, order.id, p.amount, p.method, p.finance_account_id || null, p.ref || null], tenantId
      );
    }
  }

  if (finalStatus === "CONFIRMADO") {
    const stockErrors = await validateStockForOrder(order.id, tenantId, config);
  ...
  await audit.saveAuditLogInternal({
    ...
    description: `Pedido #${order.order_number} creado. Estado: ${finalStatus}`,
  });
...
async function updateOrder(id, data, tenantId, user) {
  ...
  await db.query(
    `UPDATE orders SET status=$1, notes=$2, terms=$3, discount_amount=$4, exchange_rate=$5, updated_at=now()
     WHERE id=$6 AND tenant_id=$7`,
    [nextStatus, notes ?? order.notes, terms ?? order.terms,
     discount_amount ?? order.discount_amount, exchange_rate ?? order.exchange_rate, id, tenantId],
    tenantId
  );

  if (order.status !== "CONFIRMADO" && nextStatus === "CONFIRMADO" && !order.inventory_applied) {
    const config = await getTenantConfig(tenantId);
    const errors = await validateStockForOrder(id, tenantId, config);
    if (errors.length > 0) throw new Error(`Stock insuficiente:\n${errors.join("\n")}`);
    await applyInventoryDeduction(id, tenantId, config);
  }

  await audit.saveAuditLogInternal({
    tenant_id: tenantId, user_id: user?.id, user_name: user?.name,
    module: "VENTAS", action: "UPDATE_ORDER",
    description: `Pedido #${order.order_number || id} → ${nextStatus}`,
  });

  return getOrderFullById(id, tenantId);
}
...
async function addPaymentToOrder(data, tenantId) {
  ...
  const r = await db.query(
    ...
    [tenantId, customer_id, order_id, amount, method, finance_account_id || null, ref || null, paid_at],
    tenantId
  );
  return r.rows[0];
}
